...
=======
    ...
    includeCode = False
    includeImage = False
    errorMessage = 'Usage: downloader.py -e <email> -p <password> [-f <formats> -d <directory> --include-code --include-image]'

    # get the command line arguments/options
    try:
        opts, args = getopt.getopt(argv,"ce:p:d:f:i",["email=","pass=","directory=","formats=","include-code","inlucde-image"])
    except getopt.GetoptError:
        print errorMessage
        sys.exit(2)
    ...
    # we're in, start downloading
        ...
        for book in book_nodes:
            ...
            if not os.path.exists(path):
                ...
                # code
                if len(image) > 0 and includeImage:
                    filename = os.path.join(path, title + " [IMAGE].jpg")
                    print "Downloading IMAGE:", image[0]
                    download_to_file(filename, "https:" + image[0], session, headers, False)
>>>>>>> df52e03f0a8477a0b5572ee8f74be59071778c1d
